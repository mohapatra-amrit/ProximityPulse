...
void drawPoints(cv::Mat& pane, std::vector<int>& points) {
	...
	for (int i = 0; i < points.size(); i++)
	{
		...
		cv::putText(pane, ss.c_str(), cv::Point(points[i] + 25, y - 30), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_WHITE, 1);
	}
}
...
void drawDistLines(cv::Mat& pane, int currentX, int currentDist, int minDistX, int minDist) {
	...
	if (currentX >= 0)
	{
		...
		cv::putText(pane, ss.c_str(), cv::Point(currentX + 23 + currentDist / 2, yLineCurr - 15), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_GREEN, 1);
		cv::putText(pane, "Current: ", cv::Point(5, yLineCurr - 5), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_GREEN, 1);
	}
	...
	cv::putText(pane, ss.c_str(), cv::Point(minDistX + 23 + minDist / 2, yLineMin - 15), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_ORANGE, 1);
	cv::putText(pane, "Minimum: ", cv::Point(5, yLineMin - 5), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_ORANGE, 1);
}
