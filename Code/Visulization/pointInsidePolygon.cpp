...
void drawArrowedLine(cv::Mat& pane, cv::Point& p1, cv::Point& p2, int directionLabel) {
	...
	cv::putText(pane, directionLabel < 0 ? "L" : "R", cv::Point(middle.x + perpVector.x * 15, middle.y + perpVector.y * 15), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_GREEN_DARK, 1);
...
void drawTitledPoints(cv::Mat& pane, std::vector<cv::Point>& polygon, cv::Point& searchPoint) {
	for (int i = 0; i < polygon.size(); i++)
	{
		...
		cv::putText(pane, ss.c_str(), cv::Point(polygon[i].x - 10, polygon[i].y - 10), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_WHITE, 1);
	}
