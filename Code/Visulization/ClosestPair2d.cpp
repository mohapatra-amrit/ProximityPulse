...
void drawPoints(cv::Mat& pane, std::vector<cv::Point>& points, int startIndex, int endIndex, int pointType = 0, bool clearPane = false, bool showIndex = false) {
	...
	for (int i = startIndex; i <= endIndex; i++)
	{
		...
		if (showIndex) {
			...
			cv::putText(pane, ss.c_str(), cv::Point(points[i].x, points[i].y - 8), cv::FONT_HERSHEY_SIMPLEX, 0.4f, CL_WHITE, 1);
		}
	}
}
...
Line getShortestDistance(CP2DWrap& dataWrap, int startIndex, int endIndex) {
	...
	std::vector<cv::Point> finalPoints1(2);
	finalPoints1[0] = closestLeft.first;
	finalPoints1[1] = closestLeft.second;

	std::cout << "Left First Point is w.r.t middle Point " << dataWrap.points[middleIndex] << " is :" << finalPoints1[0] << std::endl;
	std::cout << "Left Second Point is w.r.t middle Point " << dataWrap.points[middleIndex] << " is :" << finalPoints1[1] << std::endl;

	Line closestRight = getShortestDistance(dataWrap, middleIndex + 1, endIndex);

	std::vector<cv::Point> finalPoints2(2);
	finalPoints2[0] = closestRight.first;
	finalPoints2[1] = closestRight.second;

	std::cout << "Right First Point is " << dataWrap.points[middleIndex] << " is :" << finalPoints2[0] << std::endl;
	std::cout << "Right Second Point is " << dataWrap.points[middleIndex] << " is :"<<  finalPoints2[1] << std::endl;
	...
	std::vector<cv::Point> finalPoints3(2);
	finalPoints3[0] = closestRight.first;
	finalPoints3[1] = closestRight.second;
	std::cout << "Final Closests Points in current Bounding Box is  : " << finalPoints3[0] << " " << finalPoints3[1] << std::endl;
...
void runClosestPair2D() {
	CP2DWrap dataWrap;
	dataWrap.pane = cv::Mat(700, 1100, CV_8UC3);
	...
	std::cout << "The final answer is  : " << std::endl;
	for (int i = 0; i < dataWrap.shortestDistances.size(); i++) {
		Line shortD = dataWrap.shortestDistances[i];
		std::cout << "Line with co-ordinates " << shortD.first << "," << shortD.second << " and distance between them is :" << getLineLength(shortD.first, shortD.second) << std::endl;
	}
